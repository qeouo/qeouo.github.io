...
var Ono3d = (function(){
	...
	ret.prototype.renderShader=function(shader,start,size,opFlg){
		...
		var gl = Rastgl.gl;
		if(shader.name!=="mainshader" && opFlg){
			return;
		}
		...
		while(i<start+size){
			...
			if((material.opacity<1.0) !== opFlg && shader.name==="mainshader"){
				i=j;
				continue;
			}
			...
			gl.uniform4f(unis["uPbr"],material.spc,material.rough,material.transRough,(1.0-material.ior)*0.2);
