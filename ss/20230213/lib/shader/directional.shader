...
void main(void){ 
	...
	pos.xy = unpackHalf2x16(ueye.b);
	pos.z = unpackHalf2x16(ueye.a).x;
	...
	result = basecol * max(-dot( normal ,uLight),0.0)* uLightColor*shadow_a * (1.0-specular);
		

	/*スケーリング*/ 
	//out_color = encode(vec3(0.5).rgb); 
	out_color = vec4(result,0.0)*opacity; 
