			...
			for(var j=0;j<3;j++){
				...
				if(environment.bspTree){
					...
					if(hitTriangle){
						...
						var offset = 1-Math.sign(nrm[2]);
						Vec3.mul(colz,colors[pIdx[0]][offset],ratio[0]);
						Vec3.madd(colz,colz,colors[pIdx[1]][offset],ratio[1]);
						Vec3.madd(colz,colz,colors[pIdx[2]][offset],ratio[2]);
						Vec3.madd(colz,colz,colors[pIdx[3]][offset],ratio[3]);

						offset = Math.sign(nrm[0])+2;
						Vec3.mul(colx,colors[pIdx[0]][offset],ratio[0]);
						Vec3.madd(colx,colx,colors[pIdx[1]][offset],ratio[1]);
						Vec3.madd(colx,colx,colors[pIdx[2]][offset],ratio[2]);
						Vec3.madd(colx,colx,colors[pIdx[3]][offset],ratio[3]);

						offset = (1+Math.sign(nrm[1])>>1)+4;
						Vec3.mul(coly,colors[pIdx[0]][offset],ratio[0]);
						Vec3.madd(coly,coly,colors[pIdx[1]][offset],ratio[1]);
						Vec3.madd(coly,coly,colors[pIdx[2]][offset],ratio[2]);
						Vec3.madd(coly,coly,colors[pIdx[3]][offset],ratio[3]);

						Vec3.mul(color,colx,(1.0-angx)*angy);
						Vec3.madd(color,color,coly,angx);
						Vec3.madd(color,color,colz,(1.0-angx)*(1.0-angy));
					}
				}
