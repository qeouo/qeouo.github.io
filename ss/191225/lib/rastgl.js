...
ret.init=function(_gl){
	...
	ret.commonFunction=" \n \
		...
		lowp vec4 packFloat(mediump vec3 src){ \n \
			...
			float idx = clamp(ceil(log2(m+0.001)),-16.0,15.0); \n \
			return vec4(abs(src.rgb) / exp2(idx),(dot(max(-sign(src),0.0),vec3(128.0,64.0,32.0))+(idx+16.0))/255.0); \n \
		} \n \
		mediump vec3 unpackFloat(lowp vec4 src){ \n \
			mediump vec3 rgb = src.rgb; \n \
			mediump float e = floor(src.a*255.5); \n \
			lowp vec3 s = 1.0 -floor(vec3(e/128.0,fract(e/128.0)*2.0,fract(e/64.0)*2.0))*2.0 ; \n \
			float idx = floor(fract(e/32.0) * 32.0 - 16.0); \n \
			return src.rgb * s * exp2(idx); \n \
		} \n \
