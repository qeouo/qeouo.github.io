...
var LineShader=(function(){
	...
	ret.init= function(){
		...
		shader = Ono3d.createShader("lineshader"," \
attribute vec3 aPos; \
...
,["aPos"],["uColor","projectionMatrix"]);
	...
	ret.draw=function(ono3d){
		...
		for(var i=0;i<arr.length;i++){
			...
			buff[j*3]=vertex.pos[0]
			buff[j*3+1]=vertex.pos[1]
			buff[j*3+2]=vertex.pos[2]
			j++;
...
//				gl.drawElements(gl.LINES, arrIndexes[i].size*2, gl.UNSIGNED_SHORT, arrIndexes[i].start*2*2);
				gl.drawArrays(gl.LINES, 0, (arrlength)*2);
			});
		}
	}
